...
static void retract_z_probe() {
    ...
    #else 
	// Push up the by moving the end effector, no servo needed.
	if (homing_feedrate[X_AXIS] > 40 * 60)
	{
		feedrate = 40 * 60;
	}
	else
	...
	destination[Y_AXIS] = 25;//71.7;
    destination[Z_AXIS] = current_position[Z_AXIS] + 60;
    prepare_move_raw();

	//prefent move too fast
	feedrate = homing_feedrate[Z_AXIS] / 5;
    destination[X_AXIS] = MY_X_retract_z_probe_Position ;//-67.6;
    destination[Y_AXIS] = MY_Y_retract_z_probe_Position ;//71.7;
	destination[Z_AXIS] = MY_Z_retract_z_probe_Position + 5;//30;
    prepare_move_raw();
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 29: // G29 Detailed Z-Probe, probes the bed at 3 or more points.
        {
			...
			*/
            ...
            //engage_z_probe(); //注释去掉开始的部署  // Engage Z probe by moving the end effector.
          #endif

            setup_for_endstop_move();

			feedrate = 20 * 60;//homing_feedrate[Z_AXIS];
#ifdef ACCURATE_BED_LEVELING
            ...
            #endif //NONLINEAR_BED_LEVELING mean prob from cmd
          ...
          #ifdef NONLINEAR_BED_LEVELING
            extrapolate_unprobed_bed_level();//remove
            print_bed_level();
          #else
            // solve lsq problem
          ...
          #ifndef SERVO_ENDSTOPS
			SERIAL_PROTOCOLPGM(" retract_z_probe ");
            retract_z_probe();   // Retract Z probe by moving the end effector.
          #endif //SERVO_ENDSTOPS

			//move back to first time raise
			feedrate = homing_feedrate[Z_AXIS];
			destination[X_AXIS] = 0;//-67.6;
			destination[Y_AXIS] = 0;//71.7;
			destination[Z_AXIS] = Z_RAISE_BEFORE_PROBING;//30;
			prepare_move_raw();
	...
	case 32://G32 prob center and tringle
	{

		//default reset M666 values;
		int c = -1;
		if (code_seen('c')) c = 1;
		...
		//default s for 3 times to prob 3points
		int s = 5;
		if (code_seen('S')) s = code_value();

		//default min is 0.1
		float min = 0.1;
		if (code_seen('M')) min = code_value();
		...
		for (int i = 0; i < s; i++)
		{
			...
			// center 0,0
			float center = probe_pt(0, 0, Z_RAISE_BEFORE_PROBING);
			do_blocking_move_to(0, 0, Z_RAISE_BEFORE_PROBING);

			// prob x point
			float xy = probe_pt(-DELTA_PROBABLE_RADIUS / 2 * 1.732, -DELTA_PROBABLE_RADIUS / 2, current_position[Z_AXIS] + Z_RAISE_BETWEEN_PROBINGS);
			do_blocking_move_to(-DELTA_PROBABLE_RADIUS / 2 * 1.732, -DELTA_PROBABLE_RADIUS / 2, current_position[Z_AXIS] + Z_RAISE_BETWEEN_PROBINGS);

			// prob y point
			float yz = probe_pt(DELTA_PROBABLE_RADIUS / 2 * 1.732, -DELTA_PROBABLE_RADIUS / 2, current_position[Z_AXIS] + Z_RAISE_BETWEEN_PROBINGS + 20);
			do_blocking_move_to(DELTA_PROBABLE_RADIUS / 2 * 1.732, -DELTA_PROBABLE_RADIUS / 2, current_position[Z_AXIS] + Z_RAISE_BETWEEN_PROBINGS + 20);

			// prob z point
			float zx = probe_pt(0, DELTA_PROBABLE_RADIUS, current_position[Z_AXIS] + Z_RAISE_BETWEEN_PROBINGS);
			...
			float hy = H - yz;
			float hz = H - zx;
			if (hx < min && hy < min && hz < min)
			{
				SERIAL_PROTOCOLPGM("H0:");
				SERIAL_PROTOCOL(H - center);
				break;
			}
